...
import { formatProduct } from './formatProductsHelper';
import { MAX_ITEMS_PER_CATEGORY } from './filterProductsHelper';
...
export async function getProductsById(
  ...
  return fetch(API_URL + `${category}.json`)
    .then((response) => response.json())
    .then((products: RawProduct[]) => {
      const namespaceId = products.filter(
        (product: RawProduct) => product.id === itemId,
      )[0].namespaceId;

      return products
        .filter((product: RawProduct) => product.namespaceId === namespaceId)
...
export async function getRecommendation(originalProduct: Product) {
  return getProducts().then((products) =>
    products
      .filter((product) => product.category === originalProduct.category)
